...
export default function OffersPage() {
  ...
  const { data: employeeOffersData = [], isLoading: loadingEmployees } = useEmployeeContracts(companyId || undefined);
  const { data: contractorOffersData = [], isLoading: loadingContractors } = useContractorContracts(companyId || undefined);
  ...
  // Transform and filter employee offers
  const filteredEmployeeOffers = useMemo(() => {
    return employeeOffersData
      .map(contract => {
  ...
  }, [employeeOffersData, searchTerm, selectedFilters]);

  // Transform and filter contractor offers
  const filteredContractorOffers = useMemo(() => {
    return contractorOffersData
      .map(contract => {
        const statusInfo = getStatusDisplay(null, contract.is_active);
  ...
  }, [contractorOffersData, searchTerm, selectedFilters]);

  // Calculate filter counts
  const filterCounts = useMemo(() => {
    if (activeTab === 'employees') {
      const active = employeeOffersData.filter(c => c.is_active).length;
      const pending = 0; // Would need status field
      return { accepted: active, pending };
    } else {
      const active = contractorOffersData.filter(c => c.is_active).length;
      const pending = 0;
      return { accepted: active, pending };
    }
  }, [activeTab, employeeOffersData, contractorOffersData]);

  const totalCount = activeTab === 'employees'
    ? employeeOffersData.length
    : contractorOffersData.length;
    ...
    <div className="space-y-6">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-56">
              <DropdownMenuItem asChild className="cursor-pointer">
                <Link href="/employer/offers/new" className="flex items-center gap-3 py-3">
      ...
      <div className="border-b border-[#DEE4EB]">
        <div className="flex gap-6">
          ...
          >
            Employees ({employeeOffersData.length})
          </button>
          <button
          ...
          >
            Contractors ({contractorOffersData.length})
          </button>
        </div>
      </div>
      ...
      <div className="bg-white border border-[#DEE4EB] rounded-2xl overflow-hidden">
        {activeTab === 'employees' ? (
          ...
          <table className="w-full">
            ...
            <tbody>
              {filteredEmployeeOffers.length === 0 ? (
                <tr>
                  <td colSpan={7} className="px-7 py-12 text-center text-[#8593A3]">
                    No employee offers found
                  </td>
                </tr>
              ) : (
                filteredEmployeeOffers.map((contract, index) => (
                  <tr
                    key={contract.id}
                    className={`border-b border-[#DEE4EB] hover:bg-[#F4F7FA]/50 transition-colors ${
                      index === filteredEmployeeOffers.length - 1 ? 'border-b-0' : ''
                    }`}
                  >
                    ...
                    <td className="px-7 py-5">
                      <Link href={`/employer/offers/${contract.id}`}>
              ...
              {filteredContractorOffers.length === 0 ? (
                <tr>
                  <td colSpan={8} className="px-7 py-12 text-center text-[#8593A3]">
                    No contractor offers found
                  </td>
                </tr>
              ) : (
                filteredContractorOffers.map((contract, index) => (
                  <tr
                    key={contract.id}
                    className={`border-b border-[#DEE4EB] hover:bg-[#F4F7FA]/50 transition-colors ${
                      index === filteredContractorOffers.length - 1 ? 'border-b-0' : ''
                    }`}
                  >
