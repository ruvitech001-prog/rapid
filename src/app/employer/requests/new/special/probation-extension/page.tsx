/**
 * Extension of Probation Form
 *
 * @route /employer/requests/new/special/probation-extension
 */
...
const EXTENSION_REASONS = [
  { value: 'performance_review', label: 'Performance review needed' },
  { value: 'training_incomplete', label: 'Training incomplete' },
  { value: 'project_assessment', label: 'Project assessment pending' },
...
export default function ProbationExtensionPage() {
  ...
  const currentLastDate = new Date('2022-12-11')
  const newEndDate = extensionDuration
    ? addDays(currentLastDate, parseInt(extensionDuration))
    : null
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedEmployee || !extensionDuration || !reason) {
    ...
    try {
      const newRequest = {
        ...
        request_type: 'extension_of_probation',
        title: `Probation extension - ${selectedEmployee.first_name} ${selectedEmployee.last_name}`,
        description: `${extensionDuration} days extension`,
        request_data: {
          employee_id: selectedEmployee.id,
          employee_name: `${selectedEmployee.first_name} ${selectedEmployee.last_name}`,
          current_last_date: format(currentLastDate, 'yyyy-MM-dd'),
      ...
      <div className="p-6 max-w-2xl mx-auto">
        ...
        <form onSubmit={handleSubmit}>
          <div className="bg-white rounded-xl border border-[#DEE4EB] p-6 space-y-6">
            ...
            {selectedEmployee && (
              <div className="space-y-1.5">
                <Label className="text-sm font-medium text-gray-900">Current last date</Label>
                <div className="h-10 px-3 py-2 rounded-lg border border-[#DEE4EB] bg-gray-50 text-sm text-gray-900 flex items-center">
                  {format(currentLastDate, 'dd/MMM/yyyy')}
                </div>
              </div>
            )}
            ...
            <div className="space-y-1.5">
              <Label className="text-sm font-medium text-gray-900">
                Extension of duration<span className="text-red-500">*</span>
              </Label>
            ...
            {newEndDate && (
              <div className="space-y-1.5">
                <Label className="text-sm font-medium text-gray-900">New probation end date</Label>
                <div className="h-10 px-3 py-2 rounded-lg border border-[#DEE4EB] bg-gray-50 text-sm text-gray-900 flex items-center">
            ...
            <div className="space-y-1.5">
              <Label className="text-sm font-medium text-gray-900">
                Reason of extension<span className="text-red-500">*</span>
              </Label>
